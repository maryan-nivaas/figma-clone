...
export function colorToRgba(color: Color, alpha: number) {
  return `rgba(${color.r}, ${color.g}, ${color.b}, ${alpha})`;
}

export function shadowToCss(shadow?: LayerShadow) {
  if (!shadow?.enabled) return undefined;
  const opacity = Math.max(0, Math.min(1, shadow.opacity / 100));
  return `drop-shadow(${shadow.x}px ${shadow.y}px ${shadow.blur}px ${colorToRgba(shadow.color, opacity)})`;
}
